...
spec = do
    ...
    describe "Thing Parser" $ do
        ...
        it "(Morphism) should parse named morphisms" $ do
            parseCategoryString "AB:(`a -> `b)" `shouldBe` Variable {name = Name "AB", placeholder_type = Label, placeholder_category = Composite {composite_type = Tuple, inner_categories = [Composite {composite_type = Function, inner_categories = [Thing {name = Name "a"},Thing {name = Name "b"}]}]}}
        it "(Morphism) should not catch label" $ do
            parseCategoryString "AB:`a -> `b" `shouldBe` Composite {composite_type = Function, inner_categories = [Variable {name = Name "AB", placeholder_type = Label, placeholder_category = Thing {name = Name "a"}},Thing {name = Name "b"}]}
        ...
        it "(Variable) should parse placeholders" $ do
            parseCategoryString "x@`a" `shouldBe` Variable{name=Name "x", placeholder_type=Element, placeholder_category=Thing (Name "a")}
            parseCategoryString "x@(`a)" `shouldBe` Variable {name = Name "x", placeholder_type = Element, placeholder_category = Composite {composite_type = Tuple, inner_categories = [Thing {name = Name "a"}]}}
        it "(Variable) should parse labels" $ do
            parseCategoryString "x:`a" `shouldBe` Variable{name=Name "x", placeholder_type=Label, placeholder_category=Thing (Name "a")}
            parseCategoryString "x:(`a)" `shouldBe` Variable {name = Name "x", placeholder_type = Label, placeholder_category = Composite {composite_type = Tuple, inner_categories = [Thing {name = Name "a"}]}}
        it "(Variable) should parse nested labels" $ do
            parseCategoryString "x:(x:`a)" `shouldBe` Variable {name = Name "x", placeholder_type = Label, placeholder_category = Composite {composite_type = Tuple, inner_categories = [Variable {name = Name "x", placeholder_type = Label, placeholder_category = Thing {name = Name "a"}}]}}
        it "(Refinement) should parse refinement" $ do
            parseCategoryString "{x@(`a) | `a -> `b}" `shouldBe` Refined {base = Variable {name = Name "x", placeholder_type = Element, placeholder_category = Composite {composite_type = Tuple, inner_categories = [Thing {name = Name "a"}]}}, predicate = Composite {composite_type = Function, inner_categories = [Thing {name = Name "a"},Thing {name = Name "b"}]}}
        it "(Special) should parse flexible" $ do
            parseCategoryString "(%)" `shouldBe` Special{special_type=Flexible}
            parseCategoryString "test:(%)" `shouldBe` Variable {name = Name "test", placeholder_type=Label, placeholder_category = Special {special_type = Flexible}}
        it "(Special) should parse Any" $ do
            parseCategoryString  "Any" `shouldBe` Special{special_type=Any}
            parseCategoryString "test:Any" `shouldBe` Variable {name = Name "test", placeholder_type=Label, placeholder_category = Special {special_type = Any}}
        ...
        it "(Call) should parse tuple call" $ do
            parseCategoryString "tail: $List[$list_type]" `shouldBe` Variable {name = Name "tail", placeholder_type=Label, placeholder_category = FunctionCall {base = Reference {name = Name "List"}, argument = Reference {name = Name "list_type"}}}
        ...
        it "(Recursive) should parse recursion" $ do
            parseCategoryString "self:(`a -> $self[`b])" `shouldBe`  Variable {name = Name "self", placeholder_type = Label, placeholder_category = Composite {composite_type = Tuple, inner_categories = [Composite {composite_type = Function, inner_categories = [Thing {name = Name "a"},FunctionCall {base = Reference {name = Name "self"}, argument = Thing {name = Name "b"}}]}]}}
        ...
        it "(Define) should parse define" $ do
            parseCategoryString "define x:`something" `shouldBe` Definition{def_category=Variable{name=Name "x", placeholder_type=Label, placeholder_category=Thing (Name "something")}}
    describe "File loading" $ do
        it "should load files - test1" $ do
            result <- executeTextual (Import (Reference (Name "test.test1")))
            result `shouldBe` Right (Variable {name = Name "test1", placeholder_type = Label, placeholder_category = Composite {composite_type = Tuple, inner_categories = [Composite {composite_type = Function, inner_categories = [Variable {name = Name "x", placeholder_type = Label, placeholder_category = Thing {name = Name "something"}},Composite {composite_type = Tuple, inner_categories = [Variable {name = Name "a", placeholder_type = Label, placeholder_category = Thing {name = Name "1"}},Variable {name = Name "b", placeholder_type = Label, placeholder_category = Reference {name = Name "x"}}]}]}]}})
        ...
        it "should load files - test3" $ do
            ...
            result `shouldBe`  Right (Variable {name = Name "test_define", placeholder_type = Label, placeholder_category = Composite {composite_type = Tuple, inner_categories = [Composite {composite_type = Function, inner_categories = [Definition {def_category = Variable {name = Name "x", placeholder_type = Label, placeholder_category = Thing {name = Name "something"}}},Reference {name = Name "x"}]}]}})
        it "should load dirs" $ do
            result <- executeTextual (Import (Reference (Name "test")))
            result `shouldBe` Right (Composite {composite_type = Tuple, inner_categories = [Variable {name = Name "test1", placeholder_type = Label, placeholder_category = Import {import_category = Reference {name = Name "test.test1"}}},Variable {name = Name "test2", placeholder_type = Label, placeholder_category = Import {import_category = Reference {name = Name "test.test2"}}},Variable {name = Name "test_define", placeholder_type = Label, placeholder_category = Import {import_category = Reference {name = Name "test.test_define"}}}]})
        ...
        it "should load placeholders" $ do
            ...
            result1 <- executeTextual (Import (Reference (Name "test.test1")))
            result `shouldBe` Right (Variable (Name "some_name") Label (placeholder_category (fromRight (error "404") result1)))
    describe "executeTextual" $ do
        ...
        it "should evaluate labeled imports" $ do
            ...
            result <- executeTextual parsed_result
            result1 <- executeTextual (Import (Reference (Name "test.test1")))
            result `shouldBe` Right (Variable {name = Name "x", placeholder_type = Label, placeholder_category = (placeholder_category . fromRight (error "404")) result1 })