...
spec = do
  describe "Thing Parser" $ do
    ...
    it "(Things) should parse parenthesized things" $ do
      let result = parseCategoryString "(#Something)"
      shouldBe result $ Right $ Composite {composite_type = Tuple, inner_categories = [Thing {name = Name "Something"}]}
    ...
    it "(Morphism) should parse morphisms" $ do
      ...
      shouldBe result $ Right $ Composite {composite_type = Function, inner_categories = [Thing (Name "a"), Thing (Name "b")]}
    ...
    it "(Morphism) should parse named morphisms" $ do
      ...
      shouldBe result $ Right $ Placeholder {name = Name "AB", placeholder_kind = Label, placeholder_category = Composite {composite_type = Tuple, inner_categories = [Composite {composite_type = Function, inner_categories = [Thing {name = Name "a"},Thing {name = Name "b"}]}]}}
    it "(Morphism) should not catch label" $ do
      let result = parseCategoryString "AB:#a -> #b"
      shouldBe result $ Right $ Composite {composite_type = Function, inner_categories = [Placeholder {name = Name "AB", placeholder_kind = Label, placeholder_category = Thing {name = Name "a"}},Thing {name = Name "b"}]}
    ...
    it "(Composition) should parse compositions" $ do
      let result = parseCategoryStringWith pComposition "(#a -> #b;  #b -> #c)"
      case result of
        Left e -> error e
    ...
    it "(Dereference) should parse dereferences" $ do
      ...
      case result of
        Left e -> error e
        Right cat -> shouldBe cat $ Access{base=Reference{name=Name "base_ref"}, access_id=Name "name"}
    ...
    it "(Import) should parse import" $ do
      ...
      case result of
        Left e -> error e
        Right cat -> shouldBe cat $ Import {import_category = Access {base = Reference {name = Name "test"}, access_id = Name "test1"}}
