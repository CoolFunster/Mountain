    ...
    describe "Category List" $ do
        ...
        describe "list" $ do
            it "(list) should substitute type correctly on evaluate" $ do
                execute_result <- runErrorableT $ executeTextual (parseCategoryString "(import $base.linkedlist)[`1].list")
                categoryToString (replaceResolved $ fromValid execute_result) `shouldBe` "list_def:|`empty,nonempty:(`1,$list_def)|"
        ...
        describe "join" $ do
            ...
            it "(join) should properly join on empty" $ do
                ...
                categoryToString (replaceResolved $ fromValid execute_result) `shouldBe` "(`1,`empty)"
            it "(join) should properly join on length 1 list" $ do
                ...
                execute_result <- runErrorableT $ executeTextual list_on_1_join_empty
                categoryToString (replaceResolved $ fromValid execute_result) `shouldBe` "(`1,(`1,`empty))"
        ...
        describe "head" $ do
            it "should be accessed properly" $ do
                ...
                categoryToString (replaceResolved $ fromValid execute_result) `shouldBe` "input@((list_def:|`empty,nonempty:(`1,$list_def)|).nonempty)->(`1)::(($input).#0)"
        ...
        describe "tail" $ do
            it "should be accessed properly" $ do
                ...
                execute_result <- runErrorableT $ executeTextual test_case
                isValid execute_result `shouldBe` True
                categoryToString (replaceResolved $ fromValid execute_result) `shouldBe` "input@((list_def:|`empty,nonempty:(`1,$list_def)|).nonempty)->(list_def:|`empty,nonempty:(`1,$list_def)|)::(($input).#1)"
            it "should be returning the head elem 1" $ do
