...
parseFile fp = do
  fc <- readFile fp
  let result = parseFileStruct fc
  case result of
    Left err_str -> error err_str
    Right term -> return term

parseFileStruct :: String -> Either String Exp
parseFileStruct = parseExprWith (between sc eof pStructFile)
...
pStructFile :: Parser Exp
pStructFile = EStruct <$> some (pStructStmt <* optional sc)

pBasicStructStmt :: (Id -> a -> StructStmt) -> Parser a -> Parser StructStmt
pBasicStructStmt constr p = do
...
pStruct :: Parser Exp
pStruct = between (symbol "struct" <* sc <* symbol "{" <* sc) (sc *> symbol "}") $ EStruct <$> some (pStructStmt <* optional sc)
...
pInterface = between (symbol "interface" <* sc <* symbol "{") (sc *> symbol "}") $ TInterface <$> some (pStructStmt <* optional sc)
